...
async function getAboutData(): Promise<AboutData> {
  ...
  try {
    ...
    const res = await fetch(`${baseUrl}/api/get-content/content/about.json`);
    if (res.ok) {
      const data = await res.json();
      if (data.title) return data as AboutData;
    }
  } catch {
    // API not available — fall through to filesystem
  }

  // Fallback: read from static filesystem (local dev / build)
  try {
    ...
    // Both failed — use defaults below
  }
  
  // Default content if both API and filesystem fail
  return {
    ...
    values: [
      { title: 'Excellence', description: 'We strive for the highest standards in education and student support.' },
      { title: 'Innovation', description: 'We continuously improve our teaching methods based on the latest educational research.' },
      { title: 'Care', description: 'We genuinely care about each student\'s academic and personal growth.' },
      { title: 'Integrity', description: 'We maintain the highest ethical standards in all our interactions.' },
    ],
    achievements: [
