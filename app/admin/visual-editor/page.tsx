...
import { 
  ...
  Plus, Trash2, User, Users, ArrowUp, ArrowDown
} from 'lucide-react'
...
export default function VisualEditorPage() {
  ...
  const loadSettings = async (type: 'header' | 'footer' | 'whatsapp') => {
    setIsLoading(true)
    setMessage(null)
    try {
      const paths = {
                  ...
                  {selectedSection && (
                    <>
                      ...
                      <span className="text-purple-600 font-medium">
                        {selectedPage.sections.find(s => s.id === selectedSection)?.name || selectedSection}
                      </span>
                    </>
                  )}
                </div>
